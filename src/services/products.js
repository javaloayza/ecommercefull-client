...
export const getProducts = async (options) => {
  ...
  for (const param of Object.entries(options)) {
    //* Appending the key and value of the object to the urlString. */
...
export const postProduct = async (newProduct) => {
  ...
  /* Iterating through the newProduct object and appending the key and value to the newProductForm. */
  for (const key in newProduct) {
    if (key === 'image') {
      newProduct.image.forEach(img => newProductForm.append('image', img))
    }
